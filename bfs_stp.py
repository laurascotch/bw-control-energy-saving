...
def detect_loops(switches, net_graph):
    ...
    for entry in raw_data:
        ...
        s2 = entry['dst']['dpid'].lstrip('0')
        p2 = entry['dst']['name']
        net_graph.add_edge(s1,s2)   # for already existing edges, adding them again has no effect
        edge = f"s{s1},s{s2}"
        links[edge] = f"{p1},{p2}"
    # ==============================
    ...
    try:
        if len( list(nx.find_cycle(net_graph)) ) != 0:

            bfs_value = {}
            for s in switches:
                s_degree = net_graph.degree[str(s)]
                bfs_value[str(s)] = s_degree * 2 + host_per_switch[str(s)]
            bfs_origin = max(bfs_value, key=bfs_value.get)

            # "optimized spanning tree"
            T = nx.bfs_tree(net_graph,bfs_origin)
            new_links = list(T.edges())
